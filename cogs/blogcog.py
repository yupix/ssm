...
class BlogCog(commands.Cog):
    ...
    @blog.command()
    async def register(self, ctx):
        myresult = await db_search('channel_id', 'discord_sub_blog',
                                   f'category_id = {ctx.channel.category.id} AND channel_id = {ctx.channel.id}')
        if len(myresult) == 0:
            await db_insert(ctx, 'discord_sub_blog',
                      'server_id, category_id, user_id, channel_id, embed_color, number_of_posts, xp, level',
                      '%s, %s, %s, %s, %s, %s, %s, %s',
                      f'{ctx.guild.id}, {ctx.channel.category.id}, {ctx.author.id}, {ctx.channel.id}, 5620992, 0, 0, 1')
            await embed_send(ctx, self.bot, 0, '成功', '登録に成功しました!')
        else:
            await embed_send(ctx, self.bot, 1, 'エラー', '既に登録されているカテゴリです')
