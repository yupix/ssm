	...
	async def _add(self, ctx, note, use_category=None, category_name=None):
		...
		if variable == 0:
			...
			if use_category is not None and use_category == '-c':
				...
				if not search_category_name:
					sql = "INSERT IGNORE INTO notes_category (category_name) VALUES (%s)"
					val = f'デフォルト',
					await db_insert(sql, val)
				sql = "INSERT INTO notes_detail (server_id, user_id, content, category_name) VALUES (%s, %s, %s, %s)"
				val = (f'{ctx.guild.id}', f'{ctx.author.id}', f'{note}', 'デフォルト')
			try:
				id = await db_insert(sql, val)
				search_note_created_at = await db_reformat(await db_search(custom=f'created_at from notes_detail where id = {id}'), 0)
				print(list(itertools.chain.from_iterable(search_note_created_at)))
				...
				embed.set_footer(text=f"登録日時: {search_note_created_at}")
