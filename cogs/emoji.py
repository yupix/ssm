    ...
    async def _list(self, ctx, option: typing.Optional[str] = None):
        ...
        for i in range(len(ctx.message.guild.emojis)):
            if len(embed) < 1800:
                ...
                if option == '--all':
                    embed.add_field(name=f"" + str(ctx.message.guild.emojis[i]) + ' `:' + str(ctx.message.guild.emojis[i].name) + ':`' + '\\' + str(ctx.message.guild.emojis[i]).replace('_', '\_'),
                                    value=f"" + '`' + str(ctx.message.guild.emojis[i].url) + '`', inline=True)
                else:
                    embed.add_field(name=f"" + str(ctx.message.guild.emojis[i]) + ' `:' + str(ctx.message.guild.emojis[i].name) + ':`', value=f"" + '`' + str(ctx.message.guild.emojis[i].url) + '`',
                                    inline=True)
