...
class WarframeFissuresId(Base):
    ...
    warframe_fissures_detail = relationship("WarframeFissuresDetail", backref='warframe_fissures_id', lazy='dynamic', cascade="all, delete-orphan")


class WarframeFissuresDetail(Base):
    __tablename__ = 'warframe_fissures_detail'
    id = Column(BIGINT, primary_key=True, autoincrement=True)
    api_id = Column(VARCHAR(255), ForeignKey('warframe_fissures_id.api_id', onupdate='CASCADE', ondelete='CASCADE'), unique=True)
